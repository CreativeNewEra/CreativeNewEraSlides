        ...
        if files:
            ...
            if model_name == "flux":
                ...
                if len(safetensors_files) == 1 and len(files) == 1:
                    logger.info(
                        f"Model {model_name} has single file at {local_dir}, may need full pipeline"
                    )
                    return True  # Exists but may be incomplete
                elif (local_dir / "model_index.json").exists():
                    logger.info(
                        f"Model {model_name} complete pipeline exists at {local_dir}"
                    )
                    return True
                else:
...
def main():
    ...
    parser.add_argument(
        "--token", required=True, help="Hugging Face authentication token"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force re-download existing models"
    )
