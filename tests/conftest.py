"""Pytest configuration and fixtures for CreativeNewEraSlides tests."""
...
def mock_torch():
    ...
    ), patch("torch.cuda.memory_reserved", return_value=0), patch(
...
def mock_model_loading():
    ...
    with patch(
        "diffusers.FluxPipeline.from_pretrained", return_value=mock_pipeline
    ), patch(
        "diffusers.StableDiffusionPipeline.from_pretrained", return_value=mock_pipeline
    ), patch(
        "diffusers.FluxPipeline.from_single_file", return_value=mock_pipeline
    ), patch(
        "diffusers.StableDiffusionPipeline.from_single_file", return_value=mock_pipeline
    ):
        yield mock_pipeline
