...
def test_wan_inference_detection():
    ...
    possible_scripts = ["inference.py", "sample.py", "generate.py", "run_inference.py"]
    ...
    if found_scripts:
        ...
        print(
            "   ⚠️  No inference scripts found "
            "(this is expected if Wan2.2 not downloaded)"
        )
        return False
...
def main():
    ...
    wan_status = "✅" if wan_inference_ok else "⚠️  Expected if not downloaded"
    print(f"   Wan2.2 inference detection: {wan_status}")
